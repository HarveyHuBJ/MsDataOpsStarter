 ...
 * DESCRIPITION: 查看指定表的数据基本概况
 *   Args :  
 *       @TABLE_SCHEMA  string    Schema名称
 *       @TABLE_NAME    string    表名称
 *       @ColumnListIN  string    列名称， 逗号分隔；如果是空则表示全部列
 * REFERENCE   : https://www.sqlservercentral.com/articles/data-profiling-with-t-sql
*/

CREATE PROCEDURE [common].[usp_ProfileTable]
   @TABLE_SCHEMA NVARCHAR(20)    -- Your schema here
 , @TABLE_NAME NVARCHAR(128)      -- Your table here
 , @ColumnListIN NVARCHAR(4000) = ''    -- Enter a comma-separated list of specific columns
...
DECLARE @LobCol BIT = 0   -- Analyse all VAR(char/nchar/binary) MAX data type columns (potentially time-consuming)
